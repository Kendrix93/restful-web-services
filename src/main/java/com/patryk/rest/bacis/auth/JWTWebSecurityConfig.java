...
import com.patryk.rest.webservices.jwt.JwtTokenAuthorizationOncePerRequestFilter;
import com.patryk.rest.webservices.jwt.JwtUnAuthorizedResponseAuthenticationEntryPoint;
...
public class JWTWebSecurityConfig {
    ...
    @Bean public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.csrf(csrf -> csrf.disable()).exceptionHandling().authenticationEntryPoint(jwtUnAuthorizedResponseAuthenticationEntryPoint).and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().authorizeRequests(auth -> auth.anyRequest().authenticated());
